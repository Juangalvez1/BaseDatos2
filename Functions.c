...
float BinarySearchExchangeCurrency(FILE *fp, Sales recordExchange)
{
    unsigned int start = 0, middle = 0, end = 0, sizeOfRecord = sizeof(ExchangeRates);
    ExchangeRates currentRecord;
    fseek(fp, 0, SEEK_END);
    end = (ftell(fp) / sizeOfRecord) - 1;
    while (start <= end)
    {
        middle = start + ((end - start) / 2);
        fseek(fp, middle * sizeOfRecord, SEEK_SET);
        fread(&currentRecord, sizeOfRecord, 1, fp);
        printf("compara %s con %s \n", currentRecord.Currency, recordExchange.CurrencyCode);
        if (strcmp(currentRecord.Currency, recordExchange.CurrencyCode) == 0)
            return currentRecord.Exchange;
        else if (strcmp(currentRecord.Currency, recordExchange.CurrencyCode) >= 0)
            end = middle - 1;
        else
            start = middle + 1;
    }
