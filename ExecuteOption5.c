	...
	for (int i = 0; i < numOfCustomers; i += 1)
	{
		...
		else
		{
			...
			*/
			...
			printf("%-117s_________________________\n", "");
			printf("%-117s%-15s%.2lf\n", "", "TOTAL", totalValue);
		}
		printf("\n/--------------------------------------------------------------------------------------------------------------------------------------------------------/\n");
...
void BubbleSortOption5()
{
	...
	recordsCustomers = NULL; // Free the diamic memory where recordsCustomers where stored
	free(recordsSales);
	recordsSales = NULL; // Free the diamic memory where recordsSales where stored
	free(recordsProducts);
	recordsProducts = NULL; // Free the diamic memory where recordsProducts where stored
